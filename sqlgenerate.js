...
const {map, join, head, compose, curry, toUpper, prop, equals, isEmpty, F, isArrayLike, concat, __, pluck, contains} = require('ramda');
...
var Generator = {
  ...
  statement : {
    list : (n) => {
      ...
      _.each(n.statement, i => i.inList = true);
    ...
    select : (n) => {
      ...
      if (n.inListList) { // In a select statement, so we need to surround with ( and )
        str.push('(');
      }
      str.push('SELECT ');
      if (n.result) {
        if (n.inList) {
          _.each(n.result, i => i.inListList = true);
        }
        const results = argsList(n.result);
        str.push(`${results}${LINE_END}`);
      }
      if (n.from) {
        let from = recurser(n.from);
        if (containsSelect(from)) {
          str.push(`${INDENT}FROM (${from}) ${_.uniqueId('subquery_')}${LINE_END}`);
        } else {
          from = from.replace(/_datestable/ig, '_datesTable');
          from = from.replace(/_monthstable/ig, '_monthsTable');
          from = from.replace(/_countriestable/ig, '_countriesTable');
  ...
  identifier : {
    ...
    column : (n) => {
      ...
      if (_.isEmpty(n.name)) { // They are comparing something to "", which the parser interprets as a column reference and not an empty string comparison. Take care of that here. 
          return `'' ${alias} ${index}`;
      } else {
        return `"${n.name}" ${alias} ${index}`;
      }
